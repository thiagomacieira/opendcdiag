...
static void slice_plan_init(int max_cores_per_slice)
{
    ...
    //   * hardware threads of a given module are always sliced together, so a
    ...
    while (topology.isValid()) {     // not a loop, just so we can use break
        ...
        for (const Topology::Package &p : topology.packages) {
            ...
            for (const Topology::NumaNode &n : p.numa_domains) {
                if (n.cores.size() == 0)
                    continue;   // untested (shouldn't happen!)

                const Topology::Core *c = n.cores.data();
                const Topology::Core *end = c + n.cores.size();
                ...
                while (end - c > slice_size) {
                    const Topology::Core *e = c + slice_size;
                    while (e != end && e[-1].threads.front().module_id == e[0].threads.front().module_id)
                        ++e;
                    push_to(split, c, e);
                    c = e;
                }
